...
- **Aula 02:** Representação Simbólica — Mapeamento de variáveis de processo (sensores/atuadores) em proposições. **Entregável:** Validação do catálogo de Tags.
- **Aula 03:** Tautologias e Contradições — Formulação de equações booleanas contra estados proibidos da fábrica. **Entregável:** Tabela de Verdade dos Intertravamentos.
- **Aula 04:** Lógica Proposicional: Conectivos — Programação de blocos lógicos de permissão de partida de equipamentos. **Entregável:** Algoritmo de intertravamento preliminar.
- **Aula 05:** Formas Normais — Otimização de expressões booleanas para redução de processamento. **Entregável:** Código-fonte otimizado em Python.
- **Aula 06:** Quantificadores e Predicados — Regras de varredura global em redes de sensores ($\forall x, \exists y$). **Entregável:** Módulo de varredura de estado.
- **Aula 07:** Validade e Inferência Lógica — Testes formais de consistência da matriz de segurança do processo. **Entregável:** Prova lógica de ausência de falhas.
- **Aula 08:** Lógica de Predicados e Regras — Base de conhecimento baseada em regras SE... ENTÃO para diagnósticos. **Entregável:** Base de Regras do Sistema Especialista.
- **Aula 09:** Motor de Inferência — Implementação de algoritmo de inferência (Forward/Backward Chaining). **Entregável:** Integração do motor de diagnóstico.
...
- **Aula 11:** Grafos: Definições — Modelagem da tubulação e instrumentos como Grafo Dirigido e Ponderado. **Entregável:** Matriz de Adjacência do processo.
- **Aula 12:** Matriz de Incidência e Pesos — Atribuição de perdas de carga e tempos de transporte aos arcos. **Entregável:** Matriz de Incidência parametrizada.
- **Aula 13:** Algoritmos de Busca (BFS/DFS) — Programação da verificação de continuidade de fluxo no fluido. **Entregável:** Algoritmo de busca de caminho ativo.
- **Aula 14:** Menor Caminho (Dijkstra) — Implementação de roteamento dinâmico em caso de bloqueio na rede. **Entregável:** Motor de reencaminhamento de fluido.
- **Aula 15:** Simulação de Vazamentos — Injeção de falhas na malha e validação do desvio automático. **Entregável:** Teste de estresse em malha fechada.
- **Aula 16:** Problemas Eulerianos — Otimização do percurso do robô de inspeção da infraestrutura física. **Entregável:** Algoritmo de inspeção e manutenção.
- **Aula 17:** Problemas Hamiltonianos (PCV) — Roteamento do AGV de amostragem química pelos tanques. **Entregável:** Rota otimizada de amostragem.
- **Aula 18:** Avaliação do Módulo 2 — Apresentação do Roteador Automático de Fluidos e Logística.

### Módulo 3: Árvores & Gestão de Alarmes

- **Aula 19:** Árvores: Definições — Estruturação hierárquica das seções da fábrica no banco de dados. **Entregável:** Árvore de ativos da planta.
- **Aula 20:** Representação Binária — Implementação de Árvores Binárias de Busca (BST) para consulta de Tags. **Entregável:** Algoritmo de varredura $O(\log n)$.
- **Aula 21:** Algoritmos de Percurso — Varredura em Pré-Ordem, Em-Ordem e Pós-Ordem no banco de dados. **Entregável:** Módulo de ordenação de variáveis.
- **Aula 22:** Hierarquia de Alarmes — Mapeamento de dependência de alarmes para prevenir Alarm Flood. **Entregável:** Árvore de dependência de alertas.
- **Aula 23:** Algoritmo de Supressão — Filtragem de alarmes secundários causados por falhas raiz. **Entregável:** Filtro inteligente de alarmes.
- **Aula 24:** Priorização na IHM — Estruturação da exibição visual de alarmes críticos por severidade. **Entregável:** Interface de eventos do operador.
- **Aula 25:** Árvores de Decisão: Teoria — Mapeamento de variáveis críticas de processo em nós de decisão. **Entregável:** Modelo conceitual de Trip automático.
- **Aula 26:** Algoritmo de Trip — Programação do protocolo de paralisação de emergência (Shutdown). **Entregável:** Código do algoritmo de parada segura.
- **Aula 27:** Testes de Inundação — Injeção de 50 alarmes simultâneos e validação da supressão em tempo real. **Entregável:** Relatório de validação de estabilidade.
...
- **Aula 29:** Relações: Propriedades — Definição da matriz de acessos e permissões de usuários no SCADA. **Entregável:** Matriz de Relações de Acesso.
- **Aula 30:** Ordem Parcial (Hasse) — Modelagem da hierarquia operacional (Operador < Supervisor < Engenheiro). **Entregável:** Diagrama de Hasse na IHM.
- **Aula 31:** Relações de Equivalência — Matriz de Transição de Estados (Grafcet/SFC) para processos batelada. **Entregável:** Código do sequenciador de etapas.
- **Aula 32:** Avaliação do Módulo 3 — Apresentação das permissões
...
- Adotem mensagens de commit padronizadas (ex: `feat: adiciona algoritmo de Dijkstra` ou `docs: atualiza matriz X`).
